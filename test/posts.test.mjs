...
test("every available post builds non-empty text + a renderable card", () => {
  ...
  for (const id of ids) {
    ...
    assert.ok(p.card && ["rainbow", "channel", "riskcolor", "risklevels", "riskheat", "runningroi", "cyclesync", "rsidots", "line", "bar", "mbars", "donut", "stack", "model", "cube", "scale", "gauge", "fngdial", "heatmap", "dca", "dcaladder", "kraken"].includes(p.card.type), `valid card type for ${id}`);
  }
});
