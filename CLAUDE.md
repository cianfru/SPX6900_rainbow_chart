...
- **✅✅ AEON SUITE BUILT 2026-07-24 — a full "OpenSea/Blur on steroids" analytics track (its OWN site tab + control-panel
  tab).** Reservoir SHUT DOWN 2025-10 → **Alchemy** is the floor/owners/metadata source now (secret `ALCHEMY_KEY`, set).
  Data pipeline (all keyless/cheap except listings): **`build-aeon.mjs`** (daily Alchemy floor/owners/supply →
  `aeon.json`+`aeon-history.json`), **`build-aeon-onchain.mjs`** (transfer reconstruction from `dune/out/aeon_transfers.csv`
  → `aeon-onchain.json`: owners/HODL-age/concentration/holder-flow/**per-wallet holders top-500**; `--spx=` joins per-wallet
  SPX balances for the cross-holder skyline), **`build-aeon-sales.mjs`** (`dune/aeon_sales.sql` marketplace trades →
  `aeon-sales.json` floor+volume ETH/USD), **`build-aeon-rarity.mjs`** (Alchemy metadata → `aeon-rarity.json`, 3,333 tokens,
  11 trait types, statistical rarity; ran ✅), **`build-aeon-listings.mjs`** (OpenSea active listings × rarity →
  `aeon-listings.json`). `.github/workflows/aeon.yml` runs floor+rarity(once)+listings daily, commits + deploys. Raw
  extracts kept in `dune/out/` as provenance. SPX balance query = **`dune/aeon_spx_balances.sql`** (generated by
  `gen-aeon-spx-query.mjs`, embeds all 1,172 holders, cheap evt_Transfer net-flow — NOT balances_daily which TIMED OUT).
  **Site charts (Project Aeon tab, `AEON_GROUPS` in charts-catalog):** Floor&Sales · **Holder Skyline (3D, three.js —
  towers = AEON+SPX×duration, hover→wallet, click→Zerion; up to 500 wallets, AEON+SPX/AEON-only + exclude-single-NFT
  toggles)** · Rarity lookup ("where does it sit") · **Rarity-vs-Price deal finder** (renders NFT art at underpriced
  listings) · AEON-floor-vs-SPX (cheap/expensive vs baseline) · Holder Age · Owners · Concentration · Holder Flow.
  **Cards (1:1, via `/api/og?aeon=<id>`):** skyline (static isometric) + floor/hodl/owners/concentration/behaviour — shown
  in the control panel's **🌌 Project Aeon tab** (card grid + tweets + Copy/Open-image, for MANUAL posting).
