...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1].split("/").pop())) {
  ...
  if (arg.bundle) {
    ...
    const lastW = tl.n - 1; let ethN = 0, ethHeld = 0; const ethAges = [];
    ...
    // Solana + Base from their onchain.json (wallets carry .days)
    const chainFrom = file => { const d = JSON.parse(readFileSync(new URL(`public/${file}`, root), "utf8")); const ages = d.wallets.map(w => w.days); return { n: d.wallets.length, held: Math.round(d.wallets.reduce((a, w) => a + w.bal, 0)), medAge: med(ages), hist: histOf(ages), updated: d.updated }; };
    const sol = chainFrom("solana-onchain.json"), base = chainFrom("base-onchain.json");
    const body = `// GENERATED - SPX6900 holders across ETH/Solana/Base (current snapshot + true holder ages).\n// ETH from spx-timeline.json; Solana from solana-onchain.json; Base from base-onchain.json (Alchemy cohort).\n// Regenerate: node scripts/bot/eth-sol-2026-card.mjs --bundle · hist = % of holders per ${BIN}-day age bin.\nexport const ETH_SOL_2026 = {\n  updated: ${JSON.stringify(base.updated)}, binDays: ${BIN},\n  eth: { n: ${ethN}, held: ${Math.round(ethHeld)}, medAge: ${med(ethAges)}, hist: ${JSON.stringify(histOf(ethAges))} },\n  sol: { n: ${sol.n}, held: ${sol.held}, medAge: ${sol.medAge}, hist: ${JSON.stringify(sol.hist)} },\n  base: { n: ${base.n}, held: ${base.held}, medAge: ${base.medAge}, hist: ${JSON.stringify(base.hist)} },\n};\n`;
