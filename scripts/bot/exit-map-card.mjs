...
export function exitMapSvg(stats, opts = {}) {
  ...
  const raw = flow.days.map(([d, pr, ls]) => ({ t: Date.parse(d), pr, ls })).filter(r => Number.isFinite(r.t)).sort((a, b) => a.t - b.t);
  // Aggregate to MONTHLY buckets. The raw feed is daily/weekly — ~900 hair-thin bars that read as an
  // unreadable forest at tweet size. One wide green/red stacked bar per month keeps the story (when
  // people left, and green vs red) legible in the timeline while losing no totals.
  const byMonth = new Map();
  for (const r of raw) {
    const dt = new Date(r.t), key = Date.UTC(dt.getUTCFullYear(), dt.getUTCMonth(), 15);
    const m = byMonth.get(key) || { t: key, pr: 0, ls: 0 };
    m.pr += r.pr; m.ls += r.ls; byMonth.set(key, m);
  }
  const pts = [...byMonth.values()].sort((a, b) => a.t - b.t);
  ...
  // departure bars — one wide stacked bar per MONTH: green (profit) from the baseline, red (loss) on top
  const bw = Math.max(6, (pW / Math.max(pts.length, 1)) * 0.82), rx = Math.min(4, bw / 3);
  let waves = "";
  for (const p of pts) {
    const cx = x(p.t), yProf = yB(p.pr), yTot = yB(p.pr + p.ls);
    if (p.pr > 0) waves += `<rect x="${(cx - bw / 2).toFixed(2)}" y="${yProf.toFixed(1)}" width="${bw.toFixed(2)}" height="${(bB - yProf).toFixed(1)}" rx="${rx.toFixed(1)}" fill="${GRN}" fill-opacity="0.88"/>`;
    if (p.ls > 0) waves += `<rect x="${(cx - bw / 2).toFixed(2)}" y="${yTot.toFixed(1)}" width="${bw.toFixed(2)}" height="${(yProf - yTot).toFixed(1)}" rx="${rx.toFixed(1)}" fill="${RED}" fill-opacity="0.9"/>`;
  }

  const panelLab = "wallets leaving each month";
