...
export function riskColorSvg(price, dateStr = new Date().toISOString().slice(0, 10)) {
  ...
  const W = 1200, H = 630, mL = 84, mR = 40, mT = 76, mB = 56, pW = W - mL - mR, pH = H - mT - mB;
  ...
  for (let yr = new Date(xMin).getFullYear(); yr <= new Date(xMax).getFullYear(); yr++) {
    ...
    xlab += `<text x="${x(d).toFixed(1)}" y="${H - 38}" fill="#64748b" font-size="20" text-anchor="middle" font-family="sans-serif">${yr}</text>`;
  }
  let seg = "";
  for (let i = 0; i < rs.length - 1; i++) {
    seg += `<line x1="${x(rs[i].ts).toFixed(1)}" y1="${y(rs[i].price).toFixed(1)}" x2="${x(rs[i + 1].ts).toFixed(1)}" y2="${y(rs[i + 1].price).toFixed(1)}" stroke="${riskColor((rs[i].risk + rs[i + 1].risk) / 2)}" stroke-width="3" stroke-linecap="round"/>`;
  }
  const dc = riskColor(curRisk), px = x(xMax), py = y(price);
...
<text x="64" y="${H - 18}" fill="#475569" font-size="15" font-family="sans-serif">spx6900rainbow.xyz · not financial advice</text>
</svg>`;
}
...
export function riskLevelsSvg(price, dateStr = new Date().toISOString().slice(0, 10)) {
  ...
  const W = 1200, H = 630, mL = 84, mR = 168, mT = 76, mB = 56, pW = W - mL - mR, pH = H - mT - mB;
  ...
  for (let t = xMin; t <= xMax; t += monMs * 2) {
    xlab += `<text x="${x(t).toFixed(1)}" y="${H - 38}" fill="#64748b" font-size="18" text-anchor="middle" font-family="sans-serif">${new Date(t).toLocaleDateString("en-US", { month: "short", year: "2-digit" })}</text>`;
  }
  const dc = riskColor(curRisk), px = x(xMax), py = y(price);
  return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
<rect width="${W}" height="${H}" fill="#05050e"/>
...
<text x="64" y="${H - 18}" fill="#475569" font-size="15" font-family="sans-serif">spx6900rainbow.xyz · not financial advice · risk : price</text>
</svg>`;
}
