...
export function mvrvTrendSvg(stats, opts = {}) {
  ...
  // Valuation zones from SPX's OWN quantiles (self-consistent, no hardcoded thresholds).
  ...
  const zones = zband(q(0.85), ymax, "#f87171") + zband(q(0.60), q(0.85), "#fbbf24")
    + zband(q(0.35), q(0.60), "#4ade80") + zband(q(0.15), q(0.35), "#38bdf8") + zband(ymin, q(0.15), "#818cf8");

  let grid = "";
  for (const v of [0.25, 0.5, 1, 2, 3, 5].filter(v => v >= ymin && v <= ymax)) {
    const yy = y(v).toFixed(1);
    grid += `<line x1="${mL}" y1="${yy}" x2="${W - mR}" y2="${yy}" stroke="rgba(255,255,255,0.06)"/><text x="${mL - 10}" y="${(+yy + 6).toFixed(1)}" fill="#64748b" font-size="20" text-anchor="end" font-family="sans-serif">${fMvrv(v)}</text>`;
  }
  let xlab = "";
  for (let yr = new Date(t0).getUTCFullYear(); yr <= new Date(t1).getUTCFullYear(); yr++) {
    const t = Date.UTC(yr, 0, 1); if (t < t0 || t > t1) continue;
    xlab += `<text x="${x(t).toFixed(1)}" y="${H - 50}" fill="#64748b" font-size="20" text-anchor="middle" font-family="sans-serif">${yr}</text>`;
  }
...
<text x="${(curX - 14).toFixed(1)}" y="${(curY - 16).toFixed(1)}" fill="${SPX_C}" font-size="22" font-weight="800" text-anchor="end" font-family="sans-serif">today ${fMvrv(mvrv)}</text>
<text x="60" y="${H - 20}" fill="#475569" font-size="18" font-family="sans-serif">${esc("spx6900rainbow.xyz · not financial advice · MVRV = price ÷ realized cost basis · zones are SPX6900's own quantiles")}</text>
</svg>`;
}
