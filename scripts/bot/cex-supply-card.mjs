...
export function cexSupplySvg(opts = {}) {
  ...
  for (let v = 0; v <= yMax; v += step) grid += `<line x1="${mL}" y1="${y(v).toFixed(1)}" x2="${W - mR}" y2="${y(v).toFixed(1)}" stroke="rgba(255,255,255,0.08)"/><text x="${mL - 14}" y="${(y(v) + 8).toFixed(1)}" fill="#aab6cc" font-size="24" text-anchor="end" font-family="sans-serif" font-weight="600">${fMlab(v)}</text>`;
  let xl = "";
