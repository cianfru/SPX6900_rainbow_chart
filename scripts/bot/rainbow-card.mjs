// Render the rainbow chart as a 1200x630 PNG card, server-side (no browser):
// build the model's band areas / center / price line as an SVG, then rasterize.
...
export function renderRainbowCard(stats) {
  ...
  const nowDay = Math.max(lastDay, stats.day);
  ...
  const price = DEFAULT_RAW.map(r => `${x(M.dayN(r.date)).toFixed(1)},${y(r.price).toFixed(1)}`).join(" ");
  ...
  const px = x(nowDay), py = y(stats.price);
...
<polyline points="${price}" fill="none" stroke="#ffffff" stroke-width="3.5"/>
...
<text x="${W - mR}" y="42" fill="${band.c}" font-size="30" font-weight="800" font-family="sans-serif" text-anchor="end">$${stats.price.toFixed(4)} · ${band.l}</text>
</svg>`;
