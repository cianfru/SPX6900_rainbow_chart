...
export function aeonOwnersSvg(data, opts = {}) {
  ...
  const cur = r.at(-1);
  const W = opts.W ?? 1200, H = opts.H ?? 630, mL = 100, mR = 176, mT = 196, mB = 92, pW = W - mL - mR, pH = H - mT - mB;
...
${aeonHeader("PROJECT AEON — OWNERS OVER TIME", "Distinct holders since mint, split by how many AEON each holds.", `${cur.owners.toLocaleString()} owners — ${cur.dist.one} hold a single AEON`, "#5eead4", F)}
...
export function aeonConcentrationSvg(data, opts = {}) {
  const r = rows(data); if (r.length < 30) return null;
  const cur = r.at(-1);
  const W = opts.W ?? 1200, H = opts.H ?? 630, mL = 100, mR = 60, mT = 196, mB = 92, pW = W - mL - mR, pH = H - mT - mB;
  const t0 = r[0].ts, t1 = cur.ts;
...
${aeonHeader("PROJECT AEON — CONCENTRATION", "Share of the collection held by the biggest wallets, over time.", `Top 10 wallets hold ${cur.top10.toFixed(0)}% · top 50 hold ${cur.top50.toFixed(0)}%`, "#fcd34d", F)}
...
export function aeonBehaviourSvg(data, opts = {}) {
  ...
  const W = opts.W ?? 1200, H = opts.H ?? 630, mL = 96, mR = 60, mT = 196, mB = 92, pW = W - mL - mR, pH = H - mT - mB;
...
${aeonHeader("PROJECT AEON — HOLDER FLOW", "Wallets joining vs leaving the holder base each month (mint excluded).", `Net ${netTot >= 0 ? "+" : ""}${netTot} wallets since mint · ${cur.entered} in / ${cur.exited} out last month`, netTot >= 0 ? "#5eead4" : "#fb7185", F)}
