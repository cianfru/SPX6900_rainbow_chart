...
//
// The manual is authored as markdown in docs/ — one source, still GitBook-shaped (SUMMARY.md is the
// table of contents, .gitbook.yaml points at the folder). This converts it to HTML once, at build
// time, so the site can host the book itself at ?view=docs:
//
...
function main() {
  ...
  const body = `// GENERATED by scripts/build-docs.mjs — do not edit.\n` +
    `// Source of truth: docs/*.md (also mirrored to GitBook). Run \`npm run build\` to regenerate.\n` +
