...
export default function WhaleMosaic({ isMobile }) {
  ...
  const model = useMemo(() => {
    ...
    // The interactive chart shows EVERY ≥100k wallet — one square each — sorted biggest buyer → flat →
    // biggest seller so the whole field reads green through dark to red. (The rotation card renders only
    // the movers for a tighter tweet; the site is the place to see all of it.)
  ...
  const sq = isMobile ? 13 : 16;   // one square per ≥100k wallet — small enough to fit the whole field
  ...
  return (
    <div style={{ maxWidth: 1180, margin: "0 auto", padding: "0 14px", fontFamily: SANS }}>
      ...
      <p style={{ fontSize: 12, color: "#7c8a9e", lineHeight: 1.55, margin: "12px 2px 0" }}>
        One square per wallet holding ≥100,000 SPX, sorted biggest buyer to biggest seller — <b style={{ color: "#22c55e" }}>green</b> accumulating,
