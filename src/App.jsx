...
function Tip({ active, payload, model }) {
  ...
  return (
    <div style={{
      background: "rgba(4,4,12,0.97)", border: "1px solid rgba(255,255,255,0.15)",
      borderRadius: 8, padding: "10px 14px", fontFamily: mono, fontSize: 11,
      color: "#cbd5e1", lineHeight: 1.7, maxWidth: 260, backdropFilter: "blur(8px)"
    }}>
      <div style={{ fontWeight: 700, color: "#f8fafc", fontSize: 12, marginBottom: 2 }}>
        {new Date(d.date).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}
        {proj && <span style={{ color: "#64748b", fontWeight: 400 }}> · projected</span>}
      </div>
      {d.price != null && (
        <>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", marginTop: 4, paddingTop: 4, fontSize: 10 }}>
        <div>Center: {fP(center)}</div>
        <div style={{ color: "#dc2626" }}>SELL (p98): {fP(sellLine)}</div>
        <div style={{ color: "#6366f1" }}>Fire (p2): {fP(fireLine)}</div>
        <div style={{ color: "#64748b", fontSize: 9, marginTop: 2 }}>Day {day}</div>
      </div>
    </div>
  );
}
...
function ModelCard({ mod, selected, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      color: selected ? "#c4b5fd" : "#cbd5e1", textAlign: "left",
      transition: "all 0.15s ease",
    }}>
      <div style={{ fontWeight: 600, fontSize: 11, marginBottom: 1 }}>{mod.name}</div>
      <div style={{ opacity: 0.6, fontSize: 9 }}>{mod.desc}</div>
      <div style={{ opacity: 0.45, fontSize: 9, marginTop: 2 }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto 16px" }}>
        ...
        <div style={{ fontFamily: mono, fontSize: 10, color: "#94a3b8", letterSpacing: 0.8, marginTop: 2 }}>
      ...
      {showAbout && (
        <div style={{
          ...
          <div style={{ fontWeight: 600, color: "#e2e8f0", marginBottom: 4, fontSize: 11 }}>Model Improvements</div>
          ...
          <p style={{ fontSize: 10, color: "#94a3b8" }}>
            ...
            Click &quot;Update Data&quot; to fetch the latest from CoinGecko (requires browser CORS access).
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto 10px", display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{
          ...
          border: "1px solid rgba(255,255,255,0.12)", borderRadius: 5, overflow: "hidden",
        }}>
          {HZ.map((h, i) => (
        ...
        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
          {TARGETS.map((t, i) => (
            <button key={i} onClick={() => tt(i)} style={{
              ...
              background: tg.has(i) ? t.c + "15" : "transparent",
        ...
        <ResponsiveContainer width="100%" height={500}>
          <ComposedChart data={data} margin={{ top: 8, right: 48, bottom: 16, left: 4 }}>
            ...
            <YAxis
              ...
              tick={{ fill: "#94a3b8", fontSize: 9, fontFamily: mono }}
              axisLine={{ stroke: "rgba(255,255,255,0.12)" }} tickLine={false} width={50}
            />
            ...
            {Array.from({ length: 9 }).map((_, i) => (
              <Area
                ...
                fillOpacity={0.35} stroke="none" isAnimationActive={false} activeDot={false}
              />
            ))}
            ...
            {TARGETS.map((t, i) => tg.has(i) && t.price <= yMax ? (
              <ReferenceLine
                ...
                strokeDasharray="5 3" strokeWidth={1} strokeOpacity={0.7}
                label={{
            ...
            <ReferenceLine
              x={last.date} stroke="rgba(255,255,255,0.25)" strokeDasharray="2 3"
              label={{ value: "NOW", position: "top", fill: "#cbd5e1", fontSize: 8, fontFamily: mono }}
            />

            <Line
              dataKey="reg" stroke="rgba(255,255,255,0.3)" strokeWidth={1}
      ...
      <div style={{
        ...
        {[...BAND_LABELS].reverse().map((b, i) => (
          <div key={i} style={{
            ...
            <div style={{ width: 8, height: 8, borderRadius: 2, background: b.c, opacity: 0.75 }} />
            {b.l}
          </div>
        ))}
      </div>
      ...
      {ms.length > 0 && (
        <div style={{ maxWidth: 1200, margin: "18px auto 0" }}>
          <div style={{
            fontFamily: mono, fontSize: 10, color: "#94a3b8", marginBottom: 6,
          ...
          <div style={{
            ...
            {ms.map((t, i) => (
              <div key={i} style={{
                background: `${t.c}0a`, border: `1px solid ${t.c}30`,
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 9, color: "#94a3b8" }}>{t.mc}</span>
                </div>
                <div style={{ fontSize: 10, color: "#94a3b8", lineHeight: 2.0 }}>
                  {[
                    ["▲", "#dc2626", "SELL", t.sell],
                    ...
                    <div key={j} style={{ display: "flex", justifyContent: "space-between" }}>
                      ...
                      <span style={{ color: "#f1f5f9", fontWeight: 600 }}>{fW(w)}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div style={{ maxWidth: 1200, margin: "14px auto 0", display: "flex", gap: 6, flexWrap: "wrap" }}>
        <div style={{
          ...
          borderRadius: 6, fontFamily: mono, fontSize: 10, color: "#94a3b8", lineHeight: 1.7,
        }}>
          <span style={{ color: "#e2e8f0", fontWeight: 600 }}>{m.name}:</span> {m.formula} · R²={m.r2.toFixed(4)} σ={m.std.toFixed(4)}
